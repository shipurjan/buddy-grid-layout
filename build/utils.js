 ...
 */
function moveElement(layout /*: Layout*/, l /*: LayoutItem*/, x /*: ?number*/, y /*: ?number*/, isUserAction /*: ?boolean*/, preventCollision /*: ?boolean*/, compactType /*: CompactType*/, cols /*: number*/, isLeftShift /*: ?boolean*/) /*: Layout*/{
  ...
  const collision = getFirstCollision(layout, {
    ...l,
    x: x ?? l.x,
    y: y ?? l.y
  });
  if (isUserAction && y !== collision?.y) return layout;
  ...
  const movingUp = compactType === "vertical" && typeof y === "number" ? oldPos.y >= y : compactType === "horizontal" && typeof x === "number" ? oldPos.x >= x : false;
  ...
  // overriden - nitesh
  if (isUserAction) {
    if (oldPos.x === x) isLeftShift = oldPos.y - (y ?? 0) <= 0;
    if (oldPos.y === y) isLeftShift = oldPos.x - (x ?? 0) <= 0;
  }
  ...
  for (let i = 0, len = collisions.length; i < len; i++) {
    ...
    if (collision.static) {
      layout = moveElementAwayFromCollision(layout, collision, l, oldPos, isUserAction, compactType, cols, isLeftShift // overriden - nitesh
      );
    } else {
      layout = moveElementAwayFromCollision(layout, l, collision, oldPos, isUserAction, compactType, cols, isLeftShift // overriden - nitesh
      );
    }
  }
...
function moveElementAwayFromCollision(layout /*: Layout*/, collidesWith /*: LayoutItem*/, itemToMove /*: LayoutItem*/, oldPos /*: { x: number, y: number }*/, isUserAction /*: ?boolean*/, compactType /*: CompactType*/, cols /*: number*/, isLeftShift /*: ?boolean*/) /*: Layout*/{
  const compactH = compactType === "horizontal";
  const preventCollision = collidesWith.static; // we're already colliding (not for static items)
  const isTileWrapping = isLeftShift ? itemToMove.x - 1 < 0 : itemToMove.x + 1 >= cols;
  const itemAtBeginningOfNextRow = isUserAction && itemToMove.x === 0 && collidesWith.w >= itemToMove.w && oldPos.y < collidesWith.y;
  const deltaShiftX = isLeftShift ? -collidesWith.w : itemAtBeginningOfNextRow ? -collidesWith.w : 1;
  const deltaShiftY = isLeftShift ? -collidesWith.h : 1;
  const x = isTileWrapping ? isLeftShift ? cols - 1 : 0 : itemToMove.x + deltaShiftX;
  const y = isTileWrapping ? itemToMove.y + deltaShiftY : itemToMove.y;
  isUserAction = false;
  return moveElement(layout, itemToMove, compactH ? x : undefined, y, isUserAction, preventCollision, compactType, cols, isLeftShift);
}
