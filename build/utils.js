...
function moveElement(layout /*: Layout*/, l /*: LayoutItem*/, x /*: ?number*/, y /*: ?number*/, isUserAction /*: ?boolean*/, preventCollision /*: ?boolean*/, compactType /*: CompactType*/, cols /*: number*/, isLeftShift /*: ?boolean*/) /*: Layout*/{
  ...
  if (isUserAction) {
    console.debug(l.y, y, collision?.y);
  }
  if (isUserAction && y !== collision?.y) {
    return layout;
  }
  ...
  for (let i = 0, len = collisions.length; i < len; i++) {
    ...
    if (collision.static) {
      layout = moveElementAwayFromCollision(layout, collision, l, isUserAction, compactType, cols, isLeftShift // overriden - nitesh
      );
    } else {
      layout = moveElementAwayFromCollision(layout, l, collision, isUserAction, compactType, cols, isLeftShift // overriden - nitesh
      );
    }
  }
...
function moveElementAwayFromCollision(layout /*: Layout*/, collidesWith /*: LayoutItem*/, itemToMove /*: LayoutItem*/, isUserAction /*: ?boolean*/, compactType /*: CompactType*/, cols /*: number*/, isLeftShift /*: ?boolean*/) /*: Layout*/{
  const compactH = compactType === "horizontal";
  const preventCollision = collidesWith.static; // we're already colliding (not for static items)
  const isTileWrapping = isLeftShift ? itemToMove.x - 1 < 0 : itemToMove.x + 1 >= cols;
  const deltaShiftX = isLeftShift ? -collidesWith.w : 1;
